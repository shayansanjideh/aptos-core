...
macro_rules! generate_error_response {
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        // Generate a From impl that takes in an AptosError and a status code
        // and returns an instance of $enum_name.
        impl From<($crate::poem_backend::AptosError, [<$enum_name Status>])>
            for $enum_name
        {
            fn from(
                (error, code): (
                    $crate::poem_backend::AptosError,
                    [<$enum_name Status>]
                )
            ) -> Self {
                match code {
                    $(
                    [<$enum_name Status>]::$name => $enum_name::$name(poem_openapi::payload::Json(error)),
                    )*
                }
            }
        }
        ...
        impl $crate::poem_backend::[<$name Error>] for $enum_name {
            fn [<$name:snake>](error: anyhow::Error) -> Self where Self: Sized {
                Self::from(($crate::poem_backend::AptosError::from(error), [<$enum_name Status>]::$name))
            }

            fn [<$name:snake _str>](error_str: &str) -> Self where Self: Sized {
                Self::from(($crate::poem_backend::AptosError::new(error_str.to_string()), [<$enum_name Status>]::$name))
            }
        }
        )*
        }
